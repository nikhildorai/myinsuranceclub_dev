...
$article_id = (isset($model['article_id']) && !empty($model['article_id'])) ? $model['article_id'] : '';?>
	<div class="panel panel-primary">
    	<div class="panel-heading">
        	<strong>
        		<span class="glyphicon glyphicon-th-list"></span> <?php echo (isset($model['article_id']) && !empty($model['article_id'])) ? 'Update Article' : 'Create Article';?> 
        	</strong>
...
<?php */ ?>			                
	...
	<?php 								}?>		
								...
								<?php //echo widget::run('multiFileUpload', array('policy_id'=>$article_id, 'policyModel'=>$model, 'modelName'=>'model','fieldName'=>'original_image','uploadType'=>'articles_original_image','multiUpload'=>"no")); ?>
				                ...
				                <div class="form-group">
				                    ...
				                    <?php 
												$folderUrl = $this->config->config['folder_path']['articles']['original_image'];
												$fileUrl = $this->config->config['url_path']['articles']['original_image'];
			            ...
			            <section class="panel panel-default">
			                <div class="panel-body">
					           ...
					           <?php 				}
								...
								<input type="hidden" id="article_id" name="model[article_id]" value="<?php echo array_key_exists( 'article_id',$model) ? $model['article_id'] : '';?>" />
								<?php  	if (isset($model['status']) && !in_array($model['status'], array( 'inactive', 'delete'))) {?>
					                        <input type="submit" name="submit" value="Submit" class="btn btn-success btn-lg  " />
					            <?php 	}else {	?>
					                		<input type="submit" name="submit" value="Submit" class="btn btn-success btn-lg  " />
					            <?php 	}	?>   
					                	<a href = "<?php echo $base_url; ?>admin/articles"  class="btn btn-lg btn-default">Cancel</a>     
							           <?php 	
