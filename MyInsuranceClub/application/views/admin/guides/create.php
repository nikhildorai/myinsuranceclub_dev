...
$article_id = (isset($model['article_id']) && !empty($model['article_id'])) ? $model['article_id'] : '';?>
	<div class="panel panel-primary">
    	<div class="panel-heading">
        	<strong>
        		<span class="glyphicon glyphicon-th-list"></span> <?php echo (isset($model['guide_id']) && !empty($model['guide_id'])) ? 'Update Guide' : 'Create Guide';?> 
        	</strong>
        	
        	<a href="<?php echo $base_url;?>admin/guides/" class="btn btn-w-md btn-gap-v btn-default btn-sm" style="float: right; margin-top: -5px;">Back</a>
		        ...
		        <div class="col-md-12">
		        	<div class="panel-body">    
			            <section class="panel panel-default">
			                <div class="panel-heading"><strong><span class="glyphicon glyphicon-th-list"></span> guides Details</strong></div>
			                <div class="panel-body">
...
<?php */ ?>			                
	...
	<?php 								}?>		
				                        	<span class="help-block" style="color:black;font-size: 12px"><?php echo base_url().'articles/';?><span class="slug"><?php echo array_key_exists( 'slug',$model) ? $this->util->getSlug($model['slug']) : '';?></span>/</span>	                        
				                        
				                    </div>
				                </div>
				                
				                
								<?php //echo widget::run('multiFileUpload', array('policy_id'=>$article_id, 'policyModel'=>$model, 'modelName'=>'model','fieldName'=>'original_image','uploadType'=>'guides_original_image','multiUpload'=>"no")); ?>
				                ...
				                <div class="form-group">
				                    ...
				                    <?php 
												$folderUrl = $this->config->config['folder_path']['guides']['original_image'];
												$fileUrl = $this->config->config['url_path']['guides']['original_image'];
			            ...
			            <section class="panel panel-default">
			                <div class="panel-body">
					           ...
					           <?php 				}
								...
								<input type="hidden" id="guide_id" name="model[guide_id]" value="<?php echo array_key_exists( 'guide_id',$model) ? $model['guide_id'] : '';?>" />
								<?php  	if (isset($model['status']) && !in_array($model['status'], array( 'inactive', 'delete'))) {?>
					                        <input type="submit" name="submit" value="Submit" class="btn btn-success btn-lg  " />
					            <?php 	}else {	?>
					                		<input type="submit" name="submit" value="Submit" class="btn btn-success btn-lg  " />
					            <?php 	}	?>   
					                	<a href = "<?php echo $base_url; ?>admin/guides"  class="btn btn-lg btn-default">Cancel</a>     
							           <?php 	
							                 if (isset($model['guide_id']) && !empty($model['guide_id']))
							                 {	
							                 	if (isset($model['status']) && !empty($model['status'])) 
							          			{
							          				if (in_array($model['status'], array( 'inactive', 'delete'))) {	?>
							          					<a style="float: right;" href="javascript:void(0);" ng-click="open()" id="deactiveCompany" data-hrefval="<?php echo $base_url;?>admin/guides/changeStatus/<?php echo $model['guide_id'];?>/active" class="btn btn-danger btn-lg" >Activate Guide</a>
							          	<?php 		}
							          				else if (in_array($model['status'], array( 'active'))) {?>
							                 			<a style="float: right;" href="javascript:void(0);" ng-click="open()" id="deactiveCompany" data-hrefval="<?php echo $base_url;?>admin/guides/changeStatus/<?php echo $model['guide_id'];?>/inactive" class="btn btn-danger btn-lg" >De-activate Guide</a>
							           <?php 		}
							          			} 
							          		}	?>
				                     </div>
