	...
	<div class="container">
		<div class="row">
			<div class="col-md-12 cus_res_pad">
				<h1 class="header_all" style="border-bottom: 2px solid #dadada; padding-bottom: 10px; margin-bottom: 20px;">Insurance Articles 
	<?php 			if ($articlesType == 'author' && !empty($author))
					{
						echo 'by '.$author['upro_first_name']; 
						echo isset($author['upro_last_name']) ? ' '.$author['upro_last_name'] : '';
					}
					else if ($articlesType == 'category' && !empty($tagDetails))
					{
...
<?php 
				...
				{
					foreach ($articlesDetails as $k1=>$v1)
					{
					...
					//	$tag = $v1['tag'];	
						$url = base_url().'articles/'.$articles['articles_slug'];
							...
							$folderUrl = $this->config->config['folder_path']['articles']['listing_image'];
							$fileUrl = $this->config->config['url_path']['articles']['listing_image'];
							$imgUrl = '';//$fileUrl.'/logo_missing_300x220.jpg';
							
							if (isset($articles['listing_image']) && !empty($articles['listing_image']))
							{
								if (file_exists($folderUrl.$articles['listing_image']))
								{
									$imgUrl = $fileUrl.$articles['listing_image'];
								}
							}						
?>
...
<?php 						if (!empty($imgUrl))
							{	?>						
								<div class="entry-thumb">
									<a href="<?php echo $url;?>"> 
										<img width="300" height="220" src="<?php echo $imgUrl;?>" class="ct-transition wp-post-image" alt="<?php echo $articles['title'];?>" title="<?php echo $articles['title'];?>"> 
									</a>
										...
										<div class="mask-wrapper">
											...
											<div class="entry-share-icons">
												...
												<span class="ct-twitter" title="Share on Twitter">
													<a href="https://twitter.com/intent/tweet?text=<?php echo $articles['title'];?>;url=<?php echo $url;?>" target="_blank">
												...
												<span class="ct-linkedin" title="Share on Linkedin">
													<a href="http://www.linkedin.com/shareArticle?title=<?php echo $articles['title'];?>;url=<?php echo $url;?>" target="_blank">
...
<?php 						}	?>							
							...
							<h1 class="entry-title" style="text-transform: none;">
								<a href="<?php echo $url;?>" rel="bookmark" title="<?php echo $articles['title'];?>"><?php echo Util::getSubStringFromString($articles['title'], 56);?></a>
							</h1>
							<div class="entry-content">
								<?php echo Util::getSubStringFromString($articles['description'], 102);?>&nbsp;
								<a class="read-more left" href="" title=""></a>
							</div>
							
							<div class="entry-meta c_clr" style="float: left; width: 100%; margin-bottom: 0px;">
								<span class="meta-date updated" style="float: left; border: none;"><i class="fa fa-clock-o"></i><?php echo $this->util->getDate($articles['publish_date'], 9)?></span> 
								<span class="meta-category" style="float: left; border: none; margin-left: 25px;"> <i class="fa fa-eye" title="views"></i> <?php echo $articles['page_view_count']?> Views </span> 
								<span class="meta-comments" style="float: left; border: none; margin-left: 25px;"> <i class="fa fa-comments"></i>with 0 comments </span>
							</div>
							
				<?php 		if ($articlesType != 'author')
							{	?>
										...
										<span class="ct-athr"> Author:&nbsp; 
											<span class="meta-author"> 
												...
												<span class="author vcard">
													<a class="url fn n" href="<?php echo base_url().'articles/author/'.$author['uacc_username'];?>" title="" rel="author"><?php echo $author['upro_first_name']; echo isset($author['upro_last_name']) ? ' '.$author['upro_last_name'] : '';?></a> 
												</span> 
											</span> <!-- .meta-author -->
										</span>		
										<!-- .ct-athr -->
