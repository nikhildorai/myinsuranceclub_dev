...
class Policy_features_travel_model EXTENDS Admin_Model{
	...
	public static function saveRecord($arrParams = array(), $modelType = 'update')
	{
		...
		if (!empty($arrParams))
		{
			...
			if ($modelType == 'create')
			{
				if ($db->db->insert('policy_features_travel', $values))
					$saveRecord = true;
			}
			else
			{
				$where = array('features_id'=> $arrParams['features_id']);
				if ($db->db->update('policy_features_travel', $values, $where))
					$saveRecord = true;
			}
		}	
		if ($saveRecord == true)
	...
	{
		return Util::getDbPrefix().'policy_features_travel';
	}
