...
class Demo_auth_model extends Admin_Model {
...
//var_dump(validation_errors(), $this->flexi_auth->login($this->input->post('login_identity'), $this->input->post('login_password'), $remember_user),$this->session->set_flashdata('message', $this->flexi_auth->get_messages()) );die;
	...
	//			$this->flexi_auth->db_column('user_acc', 'username') => $this->input->post('update_username')
			...
			// Redirect user.
			($response) ? redirect('admin/auth_public/dashboard') : redirect('admin/auth_public/update_account');
		}
		else
		{		
			// Set validation errors.
	...
	function change_password()
	{
		...
		if ($this->form_validation->run())
		{
			...
			($response) ? redirect('admin/auth_public/dashboard') : redirect('admin/auth_public/change_password');
		}
		else
		{		
			// Set validation errors.
