...
class Policy_rider_personal_accident_model EXTENDS Admin_Model{
	...
	public static function saveRecord($arrParams = array(), $modelType = 'update')
	{
		...
		if (!empty($arrParams))
		{
			...
			if ($modelType == 'create')
			{
				if ($db->db->insert('policy_rider_personal_accident', $values))
					$saveRecord = true;
			}
			else
			{
				$where = array('rider_id'=> $arrParams['rider_id']);
				if ($db->db->update('policy_rider_personal_accident', $values, $where))
					$saveRecord = true;
			}
		}	
		if ($saveRecord == true)
	...
	{
		return Util::getDbPrefix().'policy_rider_personal_accident';
	}
