<?php if(!defined("BASEPATH")){ exit("No direct script access allowed"); }
	 ...
	 */
		class MIC_Upload extends CI_Upload {
			 ...
			 */
				public function do_multi_upload($field){
					...
					foreach ($_FILES[$field]["name"] as $k1=>$v1){
						if (!empty($v1)){
							...
							{		
								...
								{					
									...
									if(!is_uploaded_file($_FILES[$field]["tmp_name"][$k1][$k2])){
										//Determine error number.
										$error = (!isset($_FILES[$field]["error"][$k1][$k2])) ? 4 : $_FILES[$field]["error"][$k1][$k2];
									...
									$this->file_temp	= $_FILES[$field]["tmp_name"][$k1][$k2];
									$this->file_size	= $_FILES[$field]["size"][$k1][$k2];
									$this->_file_mime_type($_FILES[$field], $k1);
									$this->file_type	= preg_replace("/^(.+?);.*$/", "\\1", $this->file_type);
									$this->file_type	= strtolower(trim(stripslashes($this->file_type[0]), '"'));
									$this->file_name	= $this->_prep_filename($_FILES[$field]["name"][$k1][$k2]);
									$this->file_ext		= $this->get_extension($this->file_name);
									$this->client_name	= $this->file_name;		
									$this->current_field = $k1;
								
									if (isset($this->extra_config) && !empty($this->extra_config))
									{
										$this->max_width = $this->extra_config[$k1]['max_width'];
										$this->max_height = $this->extra_config[$k1]['max_height'];
										$this->max_size = $this->extra_config[$k1]['max_size'];
										if (strpos($this->extra_config[$this->current_field]['allowed_types'],'|') !== false) 
										    $this->allowed_types = explode('|', $this->extra_config[$this->current_field]['allowed_types']);
										else 
											$this->allowed_types = $this->extra_config[$this->current_field]['allowed_types'];
										
										$this->upload_path = $this->extra_config[$k1]['upload_path'];
									}					
									if (isset($_FILES[$field]['filePath']))
									{
										echo $_FILES[$field]["filePath"][$k1];
										$this->upload_path = $_FILES[$field]["filePath"][$k1];
									}
									...
									if(!empty($this->_multi_file_name_override[$k1][$k2])){
										$this->file_name = $this->_prep_filename($this->_multi_file_name_override[$k1][$k2]);
										
										//If no extension was provided in the file_name config item, use the uploaded one.
										if(strpos($this->_multi_file_name_override[$k1][$k2], ".") === FALSE){
											$this->file_name .= $this->file_ext;
										//An extension was provided, lets have it!
										} else {
											$this->file_ext = $this->get_extension($this->_multi_file_name_override[$k1][$k2]);
										}
							...
							{
								//Was the file able to be uploaded? If not, determine the reason why.
								if(!is_uploaded_file($_FILES[$field]["tmp_name"][$k1])){
									//Determine error number.
									$error = (!isset($_FILES[$field]["error"][$k1])) ? 4 : $_FILES[$field]["error"][$k1];
								...
								//Set current file data as class variables.
								$this->file_temp	= $_FILES[$field]["tmp_name"][$k1];
								$this->file_size	= $_FILES[$field]["size"][$k1];
								$this->_file_mime_type($_FILES[$field], $k1);
								$this->file_type	= preg_replace("/^(.+?);.*$/", "\\1", $this->file_type);
								$this->file_type	= strtolower(trim(stripslashes($this->file_type), '"'));
								$this->file_name	= $this->_prep_filename($_FILES[$field]["name"][$k1]);
								$this->file_ext		= $this->get_extension($this->file_name);
								$this->client_name	= $this->file_name;		
								$this->current_field = $k1;
								...
								//If we're overriding, let's now make sure the new name and type is allowed.
								//Check if a filename was supplied for the current file. Otherwise, use it's given name.
								if(!empty($this->_multi_file_name_override[$k1])){
									$this->file_name = $this->_prep_filename($this->_multi_file_name_override[$k1]);
									
									//If no extension was provided in the file_name config item, use the uploaded one.
									if(strpos($this->_multi_file_name_override[$k1], ".") === FALSE){
										$this->file_name .= $this->file_ext;
									//An extension was provided, lets have it!
									} else {
										$this->file_ext = $this->get_extension($this->_multi_file_name_override[$k1]);
									}
