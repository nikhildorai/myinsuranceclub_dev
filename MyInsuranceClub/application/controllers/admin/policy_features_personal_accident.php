...
class Policy_features_personal_accident extends Admin_Controller {
 
    function __construct() 
    {
        parent::__construct();
		$this->load->model('policy_features_personal_accident_model');
		$this->load->model('policy_rider_personal_accident_model');
		$this->load->model('policy_variants_master_model');
	}
	
	public function personal_accident($variant_id = null)
	{
		if (!empty($variant_id))
		{
			//default variables
			...
			else 
			{
				...
				$where[0]['value'] = (int)$variant_id;
				$where[0]['compare'] = 'equal';
				$exist = $this->util->getTableData($modelName='Policy_features_personal_accident_model', $type="single", $where, $fields = array());
				...
				else 
				{
					...
					$where[0]['value'] = (int)$variant_id;
					$where[0]['compare'] = 'equal';
					$riders = $this->util->getTableData($modelName='Policy_rider_personal_accident_model', $type="all", $where, $fields = array());
					if (!empty($riders))
			...
			/*			array('field' => 'model[minimum_policy_terms]', 'label' => 'minimum policy terms', 'rules' => 'required'),
				...
				*/		);				
					...
					if ($this->form_validation->run())
					{
						...
						$recordId = $this->policy_features_personal_accident_model->saveRecord($arrParams, $modelType);	
						if ($recordId != false)
							$saveData[] = true;
						else 
							$saveData[] = false;
						...
						if (!empty($riderPost))
						{
							...
							$saveRiders = Util::saveRiderData($variant_id, $riderPost, $riderModelName = 'Policy_rider_personal_accident_model');
				...
				$this->template->write_view('content', 'admin/policy_features/personal_accident', $this->data, TRUE);
