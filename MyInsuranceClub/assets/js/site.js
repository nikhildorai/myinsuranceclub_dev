	...
	setModalContent: function(title, content) {
		...
		if (content.indexOf('id="advpolls-gchart') == -1) {
			...
			var el		= $(this);
				...
				el.find('.advpolls-percent').css({width: width});
			el.find('.advpolls-percent').text(width);
			
				if (width != '0%') {
					el.find('.advpolls-line').delay(1000).animate({width: width});
...
alert(width);
				if (width != '0%') {
					el.find('.advpolls-line').animate({width: 40});
					el.find('.advpolls-percent').animate({width: 40});
				}
			});
		});
